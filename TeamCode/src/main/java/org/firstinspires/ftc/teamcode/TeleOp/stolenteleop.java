...
@TeleOp
public class stolenteleop extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive() && !isStopRequested()) {
            ...
            Intake.intakeMotor.setPower(intakeOn ? 0.7 : 0);
            outtake.outtakeMotor.setPower(outtakeOn ? 0.8 : 0);
            ...
            if (platformOn) {
                outtake.platformServo.setPosition(1);
            } else {
                outtake.platformServo.setPosition(0);
            }

            if (Math.abs(outtake.outtakeMotor.getVelocity() - outtake.outtakeMotor.getPower() * MAX_TICKS) < Shooting.outtakeTolerance
                    && rumbleTime.seconds() > 1
                    && outtake.outtakeMotor.getPower() > 0.3) {
            ...
            telemetry.addData("Outtake Velocity", outtake.outtakeMotor.getVelocity());
